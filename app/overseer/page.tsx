...
async function getPlayers(): Promise<Player[] | null> {
  try {
    const res = await fetch(`https://mafia-game-production.up.railway.app/api/addPlayer`);
    if (!res.ok) {
...
async function deletePlayer(playerId: number): Promise<void> {
  try {
    const res = await fetch(`https://mafia-game-production.up.railway.app/api/deletePlayer?id=${playerId}`, {
      method: 'DELETE',
    });
    if (!res.ok) {
      throw new Error(`HTTP error! status: ${res.status}`);
